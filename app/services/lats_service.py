"""
Serviço LATS-P - Orquestra execução do grafo de classificação.
...
from app.api.models import PredictRequest, HitlContinueRequest, PredictResponse
from app.lats_sistema.utils.confidence import traduzir_confianca
from app.lats_sistema.utils.output_formatter import formatar_saida_final
from app.lats_sistema.config.fast_mode import SKIP_RAG_DEFAULT
...
def get_graph():
    ...
    Esta abordagem evita inicializar o grafo no import,
    reduzindo cold start em ambientes serverless.
    """
    global _graph_cache
    if _graph_cache is None:
        logger.info("Building LATS graph...")
        _graph_cache = build_graph()
        logger.info("✓ LATS graph compiled and cached")
    return _graph_cache


# ============================================================================
# EXECUÇÃO PRIMEIRA FASE - RAG + LATS com HITL
# ============================================================================
...
def executar_primeira_fase(req: PredictRequest) -> PredictResponse:
    ...
    logger.info("=" * 70)
    logger.info(f"Evento: {req.texto_evento[:100]}...")
    logger.info("=" * 70)
    ...
    if result.get("final"):
        ...
        logger.info("=" * 70)
        logger.info(f"Categoria: {result['final'].get('categoria_final')}")
        logger.info(f"Confiança: {confianca}")
        logger.info("=" * 70)
    elif result.get("hitl_required"):
        logger.info("=" * 70)
        logger.info(" ⏸️  HITL REQUERIDO - Aguardando decisão humana")
        logger.info("=" * 70)
...
def continuar_pos_hitl(req: HitlContinueRequest) -> PredictResponse:
    ...
    Returns:
        PredictResponse com resultado final
    """
    logger.info("=" * 80)
    ...
    if result.get("final"):
        ...
        logger.info("=" * 70)
        logger.info(" ✅ CLASSIFICAÇÃO PÓS-HITL CONCLUÍDA")
        logger.info("=" * 70)
        logger.info(f"Categoria: {result['final'].get('categoria_final')}")
        logger.info(f"Confiança: {confianca}")
        logger.info("=" * 70)
